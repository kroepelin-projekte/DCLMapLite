...
class AsyncTaskManager extends BasicTaskManager
{
    ...
    public function run(Bucket $bucket): void
    {
        ...
        $DIC->logger()->root()->info("Trying to call webserver");
        ...
        try {
            ...
            $DIC->logger()->root()->warning($t->getMessage());
            $DIC->logger()->root()->warning("Calling webserver failed, fallback to sync version");
            $sync_manager = new SyncTaskManager($this->persistence);
            $sync_manager->run($bucket);
            return;
        }
        $DIC->logger()->root()->info("Calling webserver successful");
    }
    ...
    public function runAsync()
    {
        ...
        $DIC->logger()->root()->info("Starting background job.");
        ...
        if (count($persistence->getBucketIdsByState(State::RUNNING)) >= $MAX_PARALLEL_JOBS) {
            $DIC->logger()->root()->info("Too many running jobs, worker going down.");
        ...
        while (true) {
            ...
            try {
                ...
                $DIC->logger()->root()->info("Exception while async computing: "
                    . $e->getMessage());
                $DIC->logger()->root()->info("Stack Trace: "
                    . $e->getTraceAsString());
            }
        }

        $DIC->logger()->root()->info("One worker going down because there's nothing left to do.");
